...
const Page = () => {
	...
	const findingUsernameSearch = useSetValueTimeout(
		findingUsernameValue,
		3000
	);
	...
	const [updatingUser, setUpdatingUser] = useState<UserPermission | null>(
		null
	);
	...
	// State delete user
	const [deletingUser, setDeletingUser] = useState<UserPermission | null>(
		null
	);
	...
	const handleDeleteUser = async () => {
		try {
			...
			await botApi.deleteUserPermission(
				botId as string,
			...
			<CustomModal
				...
				<Grid container spacing={2}>
					<Grid item xs={12}>
						...
						<Popper
							open={
								findedUsers.length > 0 && !!anchorElFindedUser
							}
							...
							{findedUsers
								.filter(
									(fUser) =>
										!selectedUsers.some(
											(sUser) => sUser._id === fUser._id
										) &&
										!userPermissions.some(
											(per) => per.user?._id === fUser._id
										)
								)
								.map((fUser) => (
									<MenuItem
