...
const chatHistoryApi = {
	getChatsByBotId(
		...
		const url = `/api/chat_history/${bot_id}`;
	...
	joinChat(bot_id: string, params?: Record<string, any>, config?: object) {
		const url = `/api/chat_history/join_chat_bot/${bot_id}`;
		for (const key in params) {
			if (params[key] === "") {
				delete params[key];
			}
	...
	deleteChat(chat_id: string, config?: object) {
		const url = `/api/chat_history/${chat_id}`;
	...
	getMessages(
		...
		const url = `/api/chat_history/${chat_id}/messages`;
		for (const key in params) {
			if (params[key] === "") {
				delete params[key];
			}
